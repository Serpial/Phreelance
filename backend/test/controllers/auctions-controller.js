...
describe("Auctions", () => {
  ...
  describe("/GET auction by ID", () => {
    ...
    it("given arbitrary auctions, get the auction", (done) => {
      chai
        .request(App)
        .get(URL_PREFIX + "/" + auction.meaningfulId)
        .end((_err, res) => {
          res.should.have.status(200);
          res.body.should.not.have.property("message");
          res.body.should.have.property("auction");
          res.body.auction.should.have.property("meaningfulId");
          res.body.auction.meaningfulId.should.equal(
            auction.meaningfulId.toString()
          );
          done();
        });
    });
  ...
  describe("/GET auction by creator ID", () => {
    ...
    before(async () => {
      ...
      await addArbitraryAuction({ creator: userId });
      await addArbitraryAuction({ creator: userId });
  ...
  describe("/DELETE auction by ID", () => {
    let auction;
    before(async () => {
      auction = await addArbitraryAuction({ isPublic: false });
    });
    it("given arbitrary auctions, get the auction", (done) => {
      chai
        .request(App)
        .delete(URL_PREFIX + "/" + auction.meaningfulId)
        .end((_err, res) => {
          res.should.have.status(200);
          res.body.should.have.property("message");
...
const addArbitraryAuction = async (args) => {
  const { creator, isPublic } = args ?? {};
